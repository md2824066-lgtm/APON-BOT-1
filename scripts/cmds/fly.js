...
module.exports = {
  ...
  onChat: async function ({ message, event, usersData }) {
    ...
    if (isNaN(bet) || bet <= 0) return message.reply("âŒ Invalid bet amount.");

    const user = await usersData.get(uid);
    const currentBal = user.money || 0;

    if (currentBal < bet) return message.reply(`âŒ Not enough balance. You have $${formatMoney(currentBal)}.`);
